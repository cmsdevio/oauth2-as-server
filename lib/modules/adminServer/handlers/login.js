...
module.exports.postLogin = async (request, reply) => {
    ...
    try {
        ...
        const generatedLoginToken = TokenUtils.generateTokenString();
        // TODO extract saltRounds to config
        const hash = bcrypt.hashSync(generatedLoginToken, 10);
        await Models.insertLoginToken(user._id, { token: hash });

        request.log([ 'admin-login' ], `Login successful for user ${ email }.`);
        request.cookieAuth.set({ email: user.email, scope: user.role, token: generatedLoginToken });
        if (next) {
