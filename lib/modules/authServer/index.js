...
exports.register = (server, options, next) => {
    ...
    server.route({
        ...
        handler: (request, reply) => {
            ...
            const req_id = TokenUtil.generateToken();
                ...
                .then((client) => {
                    if (!client) {
                        ...
                        if (requestScope) {
                            ...
                            client = Object.assign({}, client, {scope: requestScope})
                        }
                        console.log(client);
                ...
                if (err) {
                    ...
                    reqCacheClient.drop(key, (err) => {
                        ...
                        if (decision === 'approve') {
                            ...
                            const requestScope = reqData.scope ? reqData.scope.split(' ') : undefined;
                            const approvedScope = [];
                            if (requestScope) {
                            ...
                            if (reqData.response_type === 'code') {
                                ...
                                const code = TokenUtil.generateToken();

                                // TODO: persist code in Mongodb with ref to client
                                delete reqData.response_type;
                                if (requestScope) {
                                    reqData.scope = approvedScope.join(' ');
                                }
                                let ttl = new Date();
                                ttl.setDate(ttl.getDate() + 1);

                                Models
                                    .saveCode(Object.assign({}, reqData, {code: code, ttl: ttl}))
                                    .then((code) => {
                                        request.log(['oauth2-code'], `Generated and persisted code ${code}.`);
                                        const urlParsed = buildUrl(reqData.redirect_uri, {
                                            code: code.code,
                                            state: reqData.state
                                        });
                                ...
                                request.log(['oauth2-approve-error'], `Request type not supported yet: ${reqData.response_type}.`);
                                const urlParsed = buildUrl(reqData.redirect_uri, {
                    ...
                    if (grant_type === 'authorization_code') {
                        ...
                        Models
                            ...
                            .then((code) => {
                                ...
                                if (code.ttl < today) {
                                    ...
                                    const refresh_token = TokenUtil.generateToken();
                        ...
                        Models
                            ...
                            .then((token) => {
                                ...
                                const access_token = TokenUtil.generateToken();
