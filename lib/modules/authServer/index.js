...
exports.register = (server, options, next) => {
    ...
    server.route({
        ...
        handler: (request, reply) => {
            Models
                .findClients(request.auth.credentials.user_id)
                .then((clients) => {
    ...
    server.route({
        ...
        handler: (request, reply) => {
            ...
            Models
                .findClientById(client_id, request.auth.credentials.user_id)
                .then((client) => {
                    if (!client) {
                        const client_data = Object.assign({}, request.payload, {
    ...
    server.route({
        ...
        path: '/code',
        handler: (request, reply) => {
            const {code, client_id} = request.payload;
            let ttl = new Date();
                        ...
                        Models
                            ...
                            .then((data) => {
                                ...
                                if (data.ttl < today) {
                                    ...
                                    if (data.scope) {
                                        token_response.scope = data.scope;
                                    }
                                    let ttl = new Date();
                                    ttl.setDate(ttl.getDate() + 1);
