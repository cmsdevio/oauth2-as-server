const InvalidCodeError = require('./errors-utils');
...
module.exports.validateCode = (code, submittedValue, submittedClientId) => {
    ...
    if (!code) {
        throw new InvalidCodeError(`No matching code found for value: ${ submittedValue }.`);
    } else if (code.client_id !== submittedClientId) {
        throw new InvalidCodeError(`Mis-matching client ID. Got ${ submittedClientId }, expected ${ code.client_id }.`);
    } else if (code.ttl < today) {
        throw new InvalidCodeError(`Code ${ code.code } expired on ${ code.ttl }.`);
    }
};
