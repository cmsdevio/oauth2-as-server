...
module.exports.validateClient =
    (client, redirectUri, clientId, clientSecret, grantType, requestScopesArray) => {
        if (!client) {
            throw new Errors.InvalidClientError(`Unknown client ID ${ clientId }`);
        } else if (!client.isActive) {
            throw new Errors.InvalidClientError('Client is inactive', client.client_id);
        } else if (redirectUri && !client.redirect_uris.includes(redirectUri)) {
            throw new Errors.InvalidClientError(`Invalid redirect URI ${ redirectUri }`, client.client_id);
        } else if (clientSecret && clientSecret !== client.client_secret) {
            throw new Errors.InvalidClientError('Invalid client secret', client.client_id);
        } else if (grantType && !client.grant_types.includes(grantType)) {
            throw new Errors.InvalidClientError(`Invalid grant type ${ grantType }`, client.client_id);
            ...
            if (_.difference(requestScopesArray, clientScopesArray).length > 0) {
                throw new Errors.InvalidClientError(`Invalid scope(s) requested: ${ requestScopesArray }`, client.client_id);
            }
        }
    };
