...
module.exports.adminPostLogin = async (request, h) => {
    const Models = request.server.app.db;
    const options = request.server.app.oauthOptions;
    ...
    try {
        ...
        const token = TokenUtils.generateOAuthToken(null, null, email, options);
        return h.response({ token, email: user.email, role: user.role });
        ...
        return Boom.internal('Login error.');
    }
};
