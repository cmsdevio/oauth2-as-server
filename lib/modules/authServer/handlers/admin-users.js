...
module.exports.getUsers = async (request, h) => {
    ...
    try {
        ...
        return h({ listUsers: formattedListUsers });
    } catch (err) {
        request.log([ 'admin-users-error' ], `Error fetching users: ${ err }.`);
        return h(Boom.internal('Error fetching list of clients.'));
    }
};

module.exports.addUser = async (request, h) => {
    const Models = request.server.app.db;
    ...
    try {
        ...
        if (err.code === 11000) {
            return h(Boom.badRequest('User already exists.'));
        }
        return h(Boom.internal('Error creating user.'));
    }
};
