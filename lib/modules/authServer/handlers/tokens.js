...
module.exports.processTokenRequest = async (request, reply) => {
    ...
    const { clientId, client_secret } = OAuthUtils
    ...
    const redirectUris = request.payload.redirect_uris;
    const { scope } = request.payload;

    try {
        const client = await Models.findClientById(clientId);
        // Validate
        if (!Client.isClientValid(client, redirectUris, clientId, client_secret, grantType)) {
