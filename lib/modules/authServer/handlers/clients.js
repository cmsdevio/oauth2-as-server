...
module.exports.clientApproval = async (request, h) => {
    ...
    try {
        ...
        return h(Boom.badRequest('Missing reqId token.'));
    }
    try {
        Joi.assert(decision, decisionSchema);
    } catch (err) {
        return h(Boom.badRequest('Decision must be either `approve` or `deny`.'));
    }
    ...
    if (authReqData.response_type === 'code') {
        ...
        try {
            const savedCode = await Models.saveCode(Object.assign({}, authReqData, codeData));
